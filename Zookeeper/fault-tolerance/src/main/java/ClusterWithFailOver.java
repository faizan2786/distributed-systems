...
public class ClusterWithFailOver implements Watcher { // define a Zookeeper Watcher(event-handler) class
    ...
    public static void main (String[] args) throws IOException, InterruptedException, KeeperException, InsufficientResourcesException {
        ClusterWithFailOver cluster = new ClusterWithFailOver(ZNODE_ROOT);
        cluster.connectToZookeeper();
        cluster.addNewZNode();
        cluster.waitForEventThread();
    }

    public ClusterWithFailOver(String znodeRoot) throws InterruptedException, KeeperException {
        connectToZookeeper();
